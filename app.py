    ...
    def get_team_data(self, client, teamcode):
        try:
            if hasattr(client, 'CliEnts2') and client.CliEnts2 and hasattr(client, 'key') and client.key and hasattr(client, 'iv') and client.iv:
                ...
                # Xóa dữ liệu cũ trước khi gửi request mới
                client.last_team_packet = None 
                ...
                while time.time() - start_time < 8:
                    try:
                        # ĐỌC TỪ BIẾN AN TOÀN ĐÃ LƯU BỞI LUỒNG PHỤ
                        if hasattr(client, 'last_team_packet') and client.last_team_packet:
                            hex_data = client.last_team_packet
                            client.last_team_packet = None # Đọc xong thì xóa đi để tránh trùng lặp
...
if not ACCOUNTS:
    ACCOUNTS = [{'id': '4763244590', 'password': 'Senzu_999JR9R6'}]
    ...
    def Connect_SerVer_OnLine(self , Token , tok , host , port , key , iv , host2 , port2):
        ...
        while True:
            try:
                ...
                if not self.DaTa2 or len(self.DaTa2) == 0:
                    break # Server ngắt kết nối -> Thoát luồng
                    
                hex_data = self.DaTa2.hex()
                
                # BẮT GÓI TIN 0500 VÀ LƯU RA BIẾN RIÊNG CHO API ĐỌC
                if '0500' in hex_data[0:4] and len(hex_data) > 30:
                    self.last_team_packet = hex_data # API sẽ đọc biến này
                    try:
        ...
        while True:      
            try:
                ...
                if not self.DaTa or len(self.DaTa) == 0:	            		
                    ...
                    # Fix Spam: Nghỉ 3s trước khi reconnect và THÊM LỆNH RETURN để kết thúc luồng cũ
                    time.sleep(3)
                    self.Connect_SerVer(Token , tok , host , port , key , iv , host2 , port2)
                    return # RẤT QUAN TRỌNG: Ngăn chặn rò rỉ đệ quy!
